...
CREATE TABLE IF NOT EXISTS `rdc_drug` (
  ...
  `moa` TEXT DEFAULT NULL COMMENT '作用机制（MOA）',
...
CREATE TABLE IF NOT EXISTS `chemical_entity` (
  ...
  `radiochemical_method_description` TEXT DEFAULT NULL COMMENT '放射化学方法描述',
  `radiochemical_yield` TEXT DEFAULT NULL COMMENT '放射化学产率',
  `radiochemical_purity` TEXT DEFAULT NULL COMMENT '放射化学纯度',
  `specific_activity` TEXT DEFAULT NULL COMMENT '比活度（Specific activity）',
...
CREATE TABLE IF NOT EXISTS `chemical_affinity` (
  ...
  `affinity_value` VARCHAR(255) DEFAULT NULL COMMENT '亲和力数值',
...
CREATE TABLE IF NOT EXISTS `human_activity` (
  ...
  `patients` TEXT DEFAULT NULL COMMENT '受试者描述/分组信息',
  `frequency` TEXT DEFAULT NULL COMMENT '给药频次（qd/bid/weekly 等）',
  `dosage` TEXT DEFAULT NULL COMMENT '剂量及单位',
  `clinical_endpoint` TEXT DEFAULT NULL COMMENT '关键临床终点',
  `endpoint_period` TEXT DEFAULT NULL COMMENT '终点评价周期',
...
CREATE TABLE IF NOT EXISTS `animal_in_vivo_biodist` (
  ...
  `dosage_value` VARCHAR(255) DEFAULT NULL COMMENT '剂量数值',
  `dosage_unit` TEXT DEFAULT NULL COMMENT '剂量单位（MBq、mg/kg 等）',
  ...
  `tumor_retention_time` TEXT DEFAULT NULL COMMENT '肿瘤滞留时间',
  `tbr_tumor_blood` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/血液 T/B 比值',
  `tbr_tumor_muscle` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/肌肉 T/B 比值',
  `tbr_tumor_kidney` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/肾脏 T/B 比值',
  `tbr_tumor_salivary_glands` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/唾液腺 T/B 比值',
  `tbr_tumor_liver` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/肝脏 T/B 比值',
  `tbr_tumor_lung` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/肺部 T/B 比值',
  `tbr_tumor_heart` VARCHAR(255) DEFAULT NULL COMMENT '肿瘤/心脏 T/B 比值',
  `biodist_type` VARCHAR(128) DEFAULT NULL COMMENT '生物分布研究类型',
  `animal_model` TEXT DEFAULT NULL COMMENT '动物模型描述',
  `cell_lines` VARCHAR(255) DEFAULT NULL COMMENT '细胞系',
...
CREATE TABLE IF NOT EXISTS `animal_in_vivo_efficacy` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '自增主键（明细记录）',
  `study_ref_id` VARCHAR(128) NOT NULL COMMENT 'Business ID (animal_in_vivo_study.study_id)',
  `efficacy_animal_model` TEXT DEFAULT NULL COMMENT '疗效研究使用的动物模型',
  `efficacy_dosage_symbols` VARCHAR(16) DEFAULT NULL COMMENT '疗效剂量比较符号',
  `efficacy_dosage_value` TEXT DEFAULT NULL COMMENT '疗效剂量数值',
  `efficacy_dosage_unit` TEXT DEFAULT NULL COMMENT '疗效剂量单位',
