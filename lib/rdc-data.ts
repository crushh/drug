...
function mapBiodist(row: RowDataPacket) {
  return {
    ...
    tbr: {
      tumor_blood: row.tbr_tumor_blood,
      tumor_muscle: row.tbr_tumor_muscle,
      tumor_kidney: row.tbr_tumor_kidney,
      tumor_salivary_glands: row.tbr_tumor_salivary_glands,
      tumor_liver: row.tbr_tumor_liver,
      tumor_lung: row.tbr_tumor_lung,
      tumor_heart: row.tbr_tumor_heart,
    },
    biodist_result_image: (row.biodist_result_image as string) ?? null,
    biodist_description: (row.biodist_description as string) ?? null,
    cell_lines: (row.cell_lines as string) ?? null,
  };
}
